...
5. **Calendario → ✅ HECHO (2026-09-13).** Decisiones de Samir: **botón único en el inicio**, recordatorio asignado a **ambos socios**, aviso **1 día antes (09:00)**.
   - **Implementación:** kpi-card **"📅 Calendario"** en los Accesos rápidos del inicio (`exportarCalendarioICS()`), genera un archivo **`.ics`** con TODOS los pedidos con `fecha_entrega` definida y sin entregar/liquidar. Cada **VEVENT** incluye: `SUMMARY` "📦 Entrega camisas — {cliente} ({entrega_por})", `DESCRIPTION` (pedido, cliente, teléfono, entrega por, lugar, camisas, vendedor), `LOCATION`, y un `VALARM` (`TRIGGER:-PT15H` = día anterior a las 09:00, hora de Bogotá). Se descarga `Entregas_Camisas_IUB_AAAA-MM-DD.ics` y se abre en Calendario de Apple (iPhone) o Google Calendar (Android).
   - **Límite (documentado):** al ser página estática (sin servidor) no se puede "empujar" el evento al teléfono de cada socio automáticamente; cada quien abre el archivo y lo guarda en su calendario. El evento dice quién entrega.
   - `fechaIcs()` y `escIcs()` como helpers locales de la función.
