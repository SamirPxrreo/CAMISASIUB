...
50. **Sincronización entre dispositivos (2026-09-25, commit `f0c6a0f`):** Valentina podía guardar desde el teléfono y Samir no se enteraba hasta refrescar. Se añadió un módulo con **3 capas**:
    - **Realtime de Supabase** (`iniciarSync`): canal `sync-camisas-iub` suscrito a `postgres_changes` en las 4 tablas (`ventas`, `compras_proveedor`, `compra_aportes`, `liquidaciones`). Verificado en producción: la tabla ya estaba en la publicación `supabase_realtime` y el canal responde `SUBSCRIBED`. Se arranca al iniciar sesión (`syncUserRoleAndShow`) y se detiene en `handleLogout`.
    - **Al volver a la pestaña** (`visibilitychange` + `focus`): cubre el caso de cambiar de teléfono a computador, donde la pestaña llevaba rato en segundo plano.
    - **Sondeo cada 60 s** (`detectarCambios`): red de seguridad si Realtime se cae. Consulta una sola columna barata (`id, updated_at, created_at, finalizado, abono, abono_yesenia`) y compara una "firma" con la caché; solo si cambió recarga.
    - **Regla de oro:** si el usuario está escribiendo o tiene un modal abierto (`usuarioOcupado()`), **no se le refresca nada** — se le muestra un aviso discreto 🔔 abajo a la derecha con botón "Verlos", que se oculta solo a los 10 s. Si para entonces ya está libre, se aplica solo. Nunca se pierde lo que se estaba escribiendo.
